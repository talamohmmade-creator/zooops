...
async function sendInvitationEmail({ to, fullName, roleName, inviteLink, invitedBy }) {
  ...
  if (!accepted.includes(to.toLowerCase())) {
    const rejected = (result.rejected || []).join(', ') || to;
    throw new Error(`SMTP did not accept the recipient. Rejected: ${rejected}`);
  }
  console.log('Invitation email accepted by SMTP:', { to, messageId: result.messageId, response: result.response });
  return result;
}
