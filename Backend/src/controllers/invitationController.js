...
async function createInvitation(req, res) {
  try {
    ...
    try {
      ...
      await Invitation.updateMany({ email, status: 'pending', _id: { $ne: invitation._id } }, { status: 'revoked' });
      ...
      return res.status(201).json({ message: `Email failed (${emailError.message}). Copy and send the secure invite link manually.`, emailSent: false, invitation: { ...invitation.toObject(), role }, inviteLink });
    }
  } catch (error) {
